# 对比 Supabase AT 列表与 R2 houseat，从本地补传缺失图片
param(
    [string]$LocalDir = "D:\pythonProject\pythonProject\house\images",
    [string]$TargetBucket = "houseat",
    [int]$Parallel = 16,
    [switch]$DryRun
)
...
if ($todo.Count -eq 0) {
    if ($missing.Count -eq 0) {
        Write-Host "houseat complete, nothing to upload" -ForegroundColor Green
    } else {
...
Write-Host "done: ok=$ok fail=$fail noLocal=$noLocal r2Missing=$($missing.Count) supabaseTotal=$($atNames.Count)" -ForegroundColor Green
